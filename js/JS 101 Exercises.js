...
function hasVowels(x) {
    var vowelNum = x.replace(/[^aeiou]/g, "").length;
    if (vowelNum > 0){
        return  true
    }
    else {
...
function countVowels(x) {
    if (x){
        return x.replace(/[^aeiou]/g, "").length;
    }
    else {
        return false
    }
...
function removeVowels(x) {
    if (x){
        return x.replace(/[^BCDFGHJKLMNPQRSTVWXYZbcdfghjklmnpqrstwxyz]/g, "");
    }
    else {
        return false
    }
...
function startsWithVowel(x) {
    var pos1 = x.indexOf ("a"||"e"||"i"||"o"||"u"||"A"||"E"||"I"||"O"||"U", "");
    if (pos1<1){
        return true
    }
    else {
...
function endsWithVowel(x) {
    var lastLetter = x.slice(-1);
    if (lastLetter = /[^aeiou]/gi) {
        return true
    }
    else {
        return false
...
// not sure why assert(endsWithVowel("spinach"), false, "Exercise 48"), is returning as true; will need to ask later (I could not figure out the last three exercise examples because I was confused on why this wasn't working) sorry!
...
assert(endsWithVowel("spinach"), true, "Exercise 48");
...
function startsAndEndsWithVowel(x) {
    ...
    var endChar = x.slice(-1);
    var firstChar = x.slice(1);
